module.exports = {
    ...
    checkEliminar: function (req, res, producto){
        ...
        if (producto.autor!==req.session.usuario) {
            res.redirect("/homeUser?mensaje=No puedes borrar esta ofera porque no eres el propietario")
        } else if (producto.comprador!==undefined)
            res.redirect("/homeUser?mensaje=No puedes borrar esta ofera porque ya se ha vendido")
        else
            return true;
        return false;
    }
